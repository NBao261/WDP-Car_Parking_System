            ...
            <div className="p-6 bg-white relative flex-1 overflow-y-auto max-h-[60vh] sm:max-h-none scrollbar-hide">
              ...
              <div className="flex flex-col items-center mb-5 mt-2">
                ...
                <div className="text-center">
                  <div className="text-5xl font-black text-brand tracking-tighter leading-none">
                    {reservationResult.slotId?.code || '---'}
                  </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 mb-5 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-brand font-bold text-right max-w-[60%] line-clamp-1">
                    {reservationResult.facilityId?.name || facility?.name}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-sm">Loại xe</span>
                  <span className="text-brand font-bold">
                    {reservationResult.vehicleTypeId?.name ||
                      (activePlan?.vehicleTypeId as any)?.name ||
                      '---'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-sm">Biển số</span>
                  <span className="text-brand font-bold font-mono tracking-wider bg-white px-2 py-0.5 border border-slate-200 rounded">
                    {reservationResult.licensePlate}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-sm">Thời gian tới</span>
                  <span className="text-brand font-bold">
                    {new Date(reservationResult.startTime).toLocaleTimeString('vi-VN', {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </span>
                ...
                <div className="border-t border-slate-200 border-dashed pt-3 mt-1">
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="font-bold text-brand">
                      {activePlan?.rates?.[0]?.amount?.toLocaleString('vi-VN') || 0}đ
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
