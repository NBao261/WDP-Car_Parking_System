...
export const CustomDateTimePicker: React.FC<CustomDateTimePickerProps> = ({
  ...
  const initialDate =
    customTime && customTime.length >= 10 ? customTime.slice(0, 10) : days[0].dateStr;
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 snap-x [&::-webkit-scrollbar]:hidden">
        {days.map((day) => {
          ...
          return (
            ...
            >
              <span
                className={`text-[11px] font-bold uppercase tracking-wider mb-1 ${isSelected ? 'text-white/90' : 'text-slate-400'}`}
              >
                {day.dayOfWeek}
              </span>
