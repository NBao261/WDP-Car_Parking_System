...
export const BoardingPassTicket: React.FC<BoardingPassTicketProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-6 md:p-8 md:pr-14 relative bg-gradient-to-br from-white to-slate-50/50">
        <div className="flex justify-between items-start mb-6">
          <div className="flex-1">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight leading-tight mb-1">
              {reservation.facilityId?.name || 'Bãi đỗ xe'}
            </h3>
            <div className="flex items-start gap-1.5 text-slate-500 mt-2">
              ...
              <p className="text-sm line-clamp-2 leading-relaxed font-medium">
                {reservation.facilityId?.address || 'Chưa cập nhật địa chỉ'}
              </p>
            </div>
          </div>
          {/* Mobile Badge */}
          <span
            className={`md:hidden ${statusConfig.bg} ${statusConfig.color} font-bold px-3 py-1 rounded-full text-[10px] uppercase border ${statusConfig.border}`}
          >
            {statusConfig.label}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 bg-white rounded-2xl p-5 border border-slate-100 shadow-sm ring-1 ring-slate-900/5">
          <div>
            ...
            <p className="font-semibold text-slate-700 text-sm truncate mb-2">
              {reservation.vehicleTypeId?.name || 'Xe'}
            </p>
            ...
            <div className="inline-flex items-center border-[1.5px] border-slate-300 rounded shadow-sm overflow-hidden h-7 bg-white">
              ...
              <span className="px-2.5 font-mono font-bold text-slate-800 text-[13px] tracking-widest">
      ...
      <div
        className={`md:w-[32%] p-6 md:p-8 flex flex-col justify-between items-center text-center relative z-20 ${
      ...
      >
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          <span
            className={`hidden md:inline-flex items-center justify-center ${statusConfig.bg} ${statusConfig.color} font-black px-4 py-1.5 rounded-full text-xs border ${statusConfig.border} tracking-widest uppercase mb-6 shadow-sm backdrop-blur-md`}
          >
            {statusConfig.label}
          </span>
