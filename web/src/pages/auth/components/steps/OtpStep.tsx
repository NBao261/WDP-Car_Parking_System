...
export function OtpStep({ changeView }: OtpStepProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col">
      <label className="text-[12px] font-semibold text-[#062F28] mb-[6px]">Mã xác thực</label>
      <OTPInput
        value={otp}
        onChange={(val) => {
          setOtp(val);
      ...
      {status === 'error' && (
        <p className="text-red-500 text-[11px] mb-[4px] mt-1 text-center">Mã OTP không hợp lệ.</p>
      )}
