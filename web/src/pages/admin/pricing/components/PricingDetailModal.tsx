...
import { X, DollarSign, Building2, MapPin, Clock, Calendar, Car, ShieldCheck, Moon, CreditCard, TrendingUp, Info } from 'lucide-react';
...
export function PricingDetailModal({ isOpen, onClose, plan, facilities, vehicleTypes }: PricingDetailModalProps) {
  ...
  const facAddress = typeof plan.facilityId === 'object' ? (plan.facilityId as any).address : facilities.find(f => f._id === facId)?.address ?? '';
        ...
        >
          ...
          <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 text-[13px] text-gray-500">
                    <Calendar size={13} /> Ngày tạo: <span className="text-gray-700 font-medium">{new Date(plan.createdAt).toLocaleDateString('vi-VN')}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase mb-2 flex items-center gap-1.5"><Building2 size={14} /> Tòa nhà áp dụng</p>
                <div className="w-fit bg-gray-50/80 p-3.5 rounded-xl border border-gray-100">
                  ...
                  {facAddress && <p className="text-sm text-gray-800 mt-1.5 flex items-start gap-1.5"><MapPin size={14} className="mt-[3px] shrink-0 text-gray-400" /> <span className="line-clamp-2">{facAddress}</span></p>}
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase mb-1.5 flex items-center gap-1.5"><Car size={14} /> Loại xe</p>
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase mb-1.5 flex items-center gap-1.5"><CreditCard size={14} /> Loại giá</p>
                  <div className="flex items-center">
                    <span className={`font-semibold px-2.5 py-1.5 rounded-lg text-[12px] ${FEE_TYPE_BADGE[uiFeeType] ?? 'bg-gray-100 text-gray-600'}`}>
            ...
            <div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                {plan.rates.map((rate, idx) => (
                  <div key={idx} className={`p-4 flex justify-between items-center ${idx !== plan.rates.length - 1 ? 'border-b border-gray-100' : ''}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-700">
                          ...
                          {(idx === 0 && plan.feeMethod === 'duration_based' && plan.firstBlockHours && plan.firstBlockHours > 1) && ` (${plan.firstBlockHours} giờ)`}
                        </p>
                        {rate.startTime && rate.endTime && (
                          <p className="text-xs text-gray-500 mt-0.5">Thời gian: {rate.startTime} - {rate.endTime}</p>
                        )}
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="font-bold text-[#4a7c20] text-[15px]">{fmt(rate.amount)}</span>
                      <span className="text-xs text-gray-400 ml-1">/{translateUnit(rate.unit)}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {plan.gracePeriodMinutes ? (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50/50 border border-emerald-100">
                      <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0"><ShieldCheck size={16} /></div>
                      <div>
                        <p className="text-[11px] text-gray-500 font-medium">Thời gian miễn phí</p>
                        <p className="text-sm font-semibold text-emerald-700">{plan.gracePeriodMinutes} phút đầu</p>
                      </div>
                    </div>
                  ) : null}
                  {plan.maxDailyFee ? (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-50/50 border border-blue-100">
                      <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shrink-0"><TrendingUp size={16} /></div>
                      <div>
                        <p className="text-[11px] text-gray-500 font-medium">Phí trần tối đa</p>
                        <p className="text-sm font-semibold text-blue-700">{fmt(plan.maxDailyFee)} / ngày</p>
                      </div>
                    </div>
                  ) : null}
                  {plan.overnightFee > 0 && (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-indigo-50/50 border border-indigo-100">
                      <div className="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center shrink-0"><Moon size={16} /></div>
                      <div>
                        <p className="text-[11px] text-gray-500 font-medium">Phí qua đêm</p>
                        <p className="text-sm font-semibold text-indigo-700">{fmt(plan.overnightFee)} / đêm</p>
                      </div>
                    </div>
                  )}
                  {plan.overtimeFeePerHour > 0 && (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-orange-50/50 border border-orange-100">
                      <div className="w-8 h-8 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center shrink-0"><Clock size={16} /></div>
                      <div>
                        <p className="text-[11px] text-gray-500 font-medium">Phí quá giờ</p>
                        <p className="text-sm font-semibold text-orange-700">{fmt(plan.overtimeFeePerHour)} / giờ</p>
                      </div>
                    </div>
                  )}
                  {plan.lostCardFee > 0 && (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-red-50/50 border border-red-100">
                      <div className="w-8 h-8 rounded-full bg-red-100 text-red-600 flex items-center justify-center shrink-0"><CreditCard size={16} /></div>
                      <div>
                        <p className="text-[11px] text-gray-500 font-medium">Phí mất thẻ</p>
                        <p className="text-sm font-semibold text-red-700">{fmt(plan.lostCardFee)}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="px-6 pb-6 pt-2 flex justify-end bg-white">
            <button onClick={onClose} className="px-5 py-2.5 text-sm font-bold text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">
              Đóng
            </button>
          </div>
        </motion.div>
