  ...
  const { register, control, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: plan
      ...
      : {
        ...
        gracePeriodMinutes: '' as any, maxDailyFee: '' as any, firstBlockHours: '' as any,
      },
  });
        ...
        {isEdit && (
          <div className="bg-amber-50 border-b border-amber-100 px-6 py-3 flex gap-3 items-start text-amber-800">
            ...
            <div className="text-sm">
              ...
              Nếu bảng giá này <b>đã từng được áp dụng</b> cho bất kỳ lượt xe nào, hệ thống sẽ từ chối việc thay đổi giá tiền để bảo vệ dữ liệu lịch sử. Trong trường hợp đó, bạn vui lòng tạo bảng giá mới.
            </div>
          </div>
        )}

        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Loại Hình Thu Phí <span className="text-red-500">*</span></label>
              <div className="flex flex-col gap-2">
                {FEE_TYPE_OPTIONS.map(([val, label]) => (
                  <Controller key={val} control={control} name="uiFeeType" render={({ field }) => (
                    <label className={`flex items-center gap-2 border rounded-xl px-4 py-2.5 cursor-pointer transition-all text-sm font-semibold ${field.value === val ? 'border-[#d7ee46] bg-[#d7ee46]/10 text-[#060606] ring-1 ring-[#d7ee46]' : (errors.uiFeeType ? 'border-red-500 bg-white text-gray-700' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:bg-gray-50')}`}>
                      <input type="radio" className="w-4 h-4 accent-[#4a7c20]" value={val} checked={field.value === val} onChange={() => { field.onChange(val); }} />
                      {label}
                    </label>
                  )} />
                ))}
          ...
          {currentUiFeeType && (
            <>
              ...
              {currentUiFeeType === 'time_window' && (
                <button type="button" onClick={() => append({ label: '', amount: 0, unit: 'giờ', startTime: '', endTime: '' })}
            ...
            <div className="space-y-3 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
              ...
              {fields.map((field, idx) => (
                <div key={field.id} className="flex flex-wrap gap-3 items-start bg-white p-3 rounded-xl border border-gray-100 shadow-sm relative">
                  {currentUiFeeType === 'time_window' && (
                  ...
                  {(fields.length > 1 && currentUiFeeType === 'time_window') && (
                    <button type="button" onClick={() => remove(idx)} className="absolute -top-2 -right-2 bg-white border border-gray-200 text-gray-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 p-1 rounded-full shadow-sm transition-all">
                      <X size={14} />
                    </button>
                  )}
                </div>
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-[#060606] border-b pb-2">4. Phụ phí & Cài đặt nâng cao</h3>
