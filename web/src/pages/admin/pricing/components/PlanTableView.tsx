...
export function PlanTableView({ plans, facilities, vehicleTypes, onEdit, onViewDetail, onRefresh }: PlanTableProps) {
  ...
  const handleDelete = async () => {
    ...
    finally { setLoading(false); setDeletePlan(null); }
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm relative">
                ...
                >
                  ...
                  <td className="p-4">
                    {baseRate ? (
                      <div className="whitespace-nowrap">
                        ...
                        <span className="text-gray-500 text-xs ml-1">/{translateUnit(baseRate.unit)}</span>
                      </div>
                  ...
                  <td className="p-4 text-xs text-gray-500">
                    <div className="flex flex-col gap-0.5 whitespace-nowrap">
                      ...
                      {plan.lostCardFee > 0 && <span>Mất thẻ: <span className="font-medium text-gray-700">{fmt(plan.lostCardFee)}</span></span>}
      ...
      {menuState && activeMenuPlan && createPortal(
        <div 
          style={{ position: 'absolute', top: menuState.top, right: menuState.right, zIndex: 9999 }}
        ...
        >
          ...
          >
            <button onClick={() => { onEdit(activeMenuPlan); setMenuState(null); }} className="w-full text-left px-4 py-2 text-[13px] text-gray-700 hover:bg-gray-50 flex items-center gap-2">
              <Edit size={14} /> Chỉnh sửa
            </button>
            <div className="h-px bg-gray-100 mx-2 my-1" />
            {activeMenuPlan.status === 'active' ? (
              <button onClick={() => { toggle(activeMenuPlan, 'inactive'); setMenuState(null); }} className="w-full text-left px-4 py-2 text-[13px] text-orange-600 hover:bg-orange-50 flex items-center gap-2">
                <PowerOff size={14} /> Vô hiệu hóa
              </button>
            ) : (
              <button onClick={() => { toggle(activeMenuPlan, 'active'); setMenuState(null); }} className="w-full text-left px-4 py-2 text-[13px] text-emerald-600 hover:bg-emerald-50 flex items-center gap-2">
                <CheckCircle2 size={14} /> Kích hoạt
              </button>
            )}
            <div className="h-px bg-gray-100 mx-2 my-1" />
            <button onClick={() => { setDeletePlan(activeMenuPlan); setMenuState(null); }} className="w-full text-left px-4 py-2 text-[13px] text-red-600 hover:bg-red-50 flex items-center gap-2">
              <Trash2 size={14} /> Xóa
            </button>
          </motion.div>
        </div>,
