...
export function SlotFormModal({ facilityId, floorId, vehicleTypes, totalSlots, currentSlotCount, onClose, onSuccess }: SlotFormModalProps) {
  ...
  const filteredTypes = vehicleTypes.filter(vt => vt.name.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      ...
      >
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl mb-2 mt-2">
          <button onClick={() => { setMode('single'); setErrors({}); }} className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-colors ${mode === 'single' ? 'bg-[#d7ee46] shadow-sm text-[#060606]' : 'text-gray-500 hover:text-gray-700'}`}>Tạo 1 Slot</button>
          <button onClick={() => { setMode('bulk'); setErrors({}); }} className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-colors ${mode === 'bulk' ? 'bg-[#d7ee46] shadow-sm text-[#060606]' : 'text-gray-500 hover:text-gray-700'}`}>Tạo Nhiều Slot</button>
        </div>

        {/* Capacity indicator */}
        <div className={`flex items-center gap-2 text-sm mb-4 font-medium ${isFull
            ? 'text-red-600'
            : remainingSlots <= 5
              ? 'text-amber-600'
              : 'text-[#4a7c20]'
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#d7ee46] ${errors.count ? 'border-red-400' : 'border-gray-200'
                    }`}
                />
          ...
          {vehicleTypes.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2 p-1 max-h-40 overflow-y-auto">
              ...
              {filteredTypes.length === 0 && (
                <p className="text-xs text-gray-400 py-2 w-full">Không tìm thấy loại xe phù hợp với "{searchQuery}".</p>
              )}
            </div>
          )}
