            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span style={{ 
                    fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 5,
                    background: staff.status === 'active' ? '#ECFDF5' : staff.status === 'locked' ? '#FEF2F2' : '#f0f1f0',
                    color: staff.status === 'active' ? '#047857' : staff.status === 'locked' ? '#DC2626' : '#6b6e6b',
                    border: `1px solid ${staff.status === 'active' ? '#D1FAE5' : staff.status === 'locked' ? '#FEE2E2' : '#e2e3e2'}`
                  }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: staff.status === 'active' ? '#10b981' : staff.status === 'locked' ? '#EF4444' : '#9b9e9b' }} />
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 flex items-center gap-1">TÊN</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 flex items-center gap-1"><ShieldCheck size={14} /> Vai trò</p>
                    <p className="text-sm font-medium text-gray-800">{staff.role === 'staff' ? 'Nhân viên bảo vệ' : staff.role}</p> 
                </div>
                <div>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Tòa nhà được phân công ({(staff.assignedFacilities ?? []).length})</p>
                {(staff.assignedFacilities ?? []).length > 0 ? (
                  <div className="flex flex-col gap-3">
                    {(staff.assignedFacilities ?? []).map((f: any, idx: number) => {
                      ...
                      const facName = typeof f !== 'string' && (f as AssignedFacility).name ? (f as AssignedFacility).name : (facilityMap[id] || 'Tòa nhà không xác định');
