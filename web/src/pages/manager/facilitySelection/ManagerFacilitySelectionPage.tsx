...
export default function ManagerFacilitySelectionPage() {
  ...
  const handleStart = () => {
    ...
    sessionStorage.setItem("manager_facility_id", selectedFacilityId);
    sessionStorage.setItem("manager_facility_name", facilityName);
    ...
    <div className="min-h-screen bg-[#eff0ef] flex items-center justify-center font-sans text-[#060606] relative">
      ...
      <div className="bg-white p-9 rounded-[16px] shadow-[0_12px_40px_rgba(0,0,0,0.15)] w-full max-w-[420px] z-10 relative">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-[20px] font-bold">Quản Lý Cơ Sở</h1>
          <p className="text-[13px] text-[#6b6b6b] mt-1 text-center">
            Chọn bãi xe (cơ sở) bạn muốn quản lý trong phiên làm việc này.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-[#e8e9e8]">
          ...
          {assignedFacilities.length === 0 && (
            <p className="text-[12px] text-red-500 mt-1.5">
              Bạn chưa được phân công quản lý bãi xe nào. Vui lòng liên hệ Admin.
            </p>
          )}
        </div>
