...
const getActiveBadgeClasses = (checkInTime: string) => {
  ...
  if (diffHours < 24) {
    ...
    switch (steps) {
      case 0:
        return 'bg-[#9FE870]/20 text-[#062F28] border-[#9FE870]/30'; // 0-4h
      case 1:
        return 'bg-[#9FE870]/40 text-[#062F28] border-[#9FE870]/50'; // 4-8h
      case 2:
        return 'bg-[#9FE870]/60 text-[#062F28] border-[#9FE870]/70'; // 8-12h
      case 3:
        return 'bg-[#9FE870]/80 text-[#062F28] border-[#9FE870]/90'; // 12-16h
      case 4:
        return 'bg-[#9FE870] text-[#062F28] border-[#9FE870]'; // 16-20h
      default:
        return 'bg-[#8BD65E] text-[#062F28] border-[#8BD65E]'; // 20-24h (đậm hơn chút)
    }
  } else {
    ...
    switch (days) {
      case 1:
        return 'bg-[#062F28]/60 text-white border-[#062F28]/70'; // Ngày 2 (24h-48h)
      case 2:
        return 'bg-[#062F28]/80 text-white border-[#062F28]/90'; // Ngày 3 (48h-72h)
      default:
        return 'bg-[#062F28] text-white border-[#062F28]'; // Ngày 4+
    }
  }
};
...
export default function TableSessionsPage({
  ...
  const filteredSessions = sessions
    .filter(
      (s) =>
        search === '' ||
        s.licensePlate.toUpperCase().includes(search) ||
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden relative">
      <div className="px-4 py-2.5 border-b border-gray-100 flex items-center justify-between gap-4 flex-wrap bg-white">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) => {
              setFilterVehicleTypeId(e.target.value);
              setCurrentPage(1);
            }}
          ...
          <button className="p-1.5 border border-gray-200 rounded-lg text-[#060606] hover:bg-gray-50">
          ...
          <tbody className="divide-y divide-gray-50">
            {currentData.map((session, index) => {
              ...
              return (
                <tr
                  key={session._id}
                  className="hover:bg-[#f9fafb] transition-colors border-b border-gray-50 last:border-0"
                >
                  ...
                  <td className="px-4 py-2.5 font-mono text-[15px] text-[#060606] font-bold truncate">
                    {session.licensePlate}
                  </td>
                  <td className="px-4 py-2.5 text-[#060606] text-sm truncate">
                    {session.vehicleTypeId?.name || 'N/A'}
                  </td>
                  <td className="px-4 py-2.5 text-[#060606] text-sm truncate">
                    {session.floorId && session.slotId
                      ? `${session.floorId.name} - ${session.slotId.code}`
                      : '—'}
                  </td>
                  <td className="px-4 py-2.5 text-[#060606] text-sm truncate">
                    {session.gateIn || 'N/A'}
                  </td>
                  <td className="px-4 py-2.5 text-[#060606]/70 text-sm tabular-nums truncate">
                    {new Date(session.checkInTime).toLocaleString('vi-VN')}
                  </td>
                  <td className="px-4 py-2.5 text-[#060606] text-sm font-medium tabular-nums truncate">
                    {calculateDuration(session.checkInTime)}
                  </td>
      ...
      <div className="p-4 border-t border-gray-100 flex items-center justify-between bg-white">
        <div className="text-sm text-gray-500">
          Hiển thị {(currentPage - 1) * itemsPerPage + (filteredSessions.length > 0 ? 1 : 0)} đến{' '}
          {Math.min(currentPage * itemsPerPage, filteredSessions.length)} trên tổng số{' '}
          {filteredSessions.length} mục
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100 bg-gray-50">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-0.5">
                  Chi tiết của {selectedSession.code}
                </p>
                <h3 className="font-bold text-xl text-[#060606] font-mono">
                  {selectedSession.licensePlate}
                </h3>
              </div>
              <button
                onClick={() => setSelectedSession(null)}
                className="p-1.5 hover:bg-gray-200 rounded-lg mt-0.5"
              >
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="w-full bg-[#060606] rounded-2xl flex items-center justify-center py-6">
                <span className="font-mono font-bold text-4xl tracking-widest text-[#d7ee46]">
                  {selectedSession.licensePlate}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                  ...
                  <p className="font-semibold text-[#060606]">
                    {selectedSession.cardCode || selectedSession.code}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                  <p className="text-gray-400 text-xs mb-1">Loại xe</p>
                  <p className="font-semibold text-[#060606]">
                    {selectedSession.vehicleTypeId?.name || '—'}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                ...
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                  ...
                  <p className="font-semibold text-[#060606]">
                    {selectedSession.floorId && selectedSession.slotId
                      ? `${selectedSession.floorId.name} - ${selectedSession.slotId.code}`
                      : '—'}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                ...
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                  ...
                  <p className="font-semibold text-[#060606]">
                    {new Date(selectedSession.checkInTime).toLocaleString('vi-VN')}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                ...
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                  ...
                  <p className="font-semibold text-[#060606]">
                    {calculateDuration(selectedSession.checkInTime)}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-gray-100">
            ...
            <div className="p-6">
              <p className="text-gray-600 text-sm mb-5">
                Thanh toán và mở barie cho xe{' '}
                <strong className="font-mono text-lg text-[#060606] ml-1">
                  {showCheckoutModal.licensePlate}
                </strong>
              </p>
              <div className="flex justify-between items-center mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <span className="text-2xl font-bold text-[#060606]">
                  {showCheckoutModal.totalFee?.toLocaleString('vi-VN') || 0} ₫
                </span>
              </div>
              <div className="flex gap-3">
                <button
                  onClick={() => setShowCheckoutModal(null)}
                  className="flex-1 py-3 border border-gray-200 rounded-xl font-medium hover:bg-gray-50 text-[#060606]"
                >
                  Hủy bỏ
                </button>
                <button
                  onClick={() => {
                    alert('Đã thu tiền & Mở barie thành công!');
                    setShowCheckoutModal(null);
                    fetchSessions();
                  }}
                  className="flex-1 py-3 bg-[#d7ee46] rounded-xl font-semibold hover:brightness-95 text-[#060606]"
                >
                  Thu tiền & Mở Cổng
                </button>
              </div>
