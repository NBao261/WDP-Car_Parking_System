...
import { useShiftSelectionLogic } from "./components/useShiftSelectionLogic";

export default function ShiftSelectionPage() {
  const logic = useShiftSelectionLogic();
        ...
        <div className="pt-4 border-t border-[#e8e9e8]">
          ...
          <div className="relative">
            <select
              ...
              onChange={(e) => logic.setSelectedFacilityId(e.target.value)}
            ...
            >
              ...
              {logic.assignedFacilities.map((facility: any) => (
          ...
          {logic.assignedFacilities.length === 0 && (
            <p className="text-[12px] text-red-500 mt-1.5">Bạn chưa được phân công tại toà nhà nào. Vui lòng liên hệ Admin.</p>
          )}
        </div>

        <div className="flex gap-3 pt-8 mt-4">
          <button onClick={logic.handleCancel} className="flex-1 h-11 border border-[#e8e9e8] rounded-[8px] text-[#060606] font-medium hover:bg-gray-50 transition-colors">
            Hủy
          </button>
          <button onClick={logic.handleStartShift} disabled={!logic.selectedFacilityId} className="flex-[2] h-11 bg-[#1a1a1a] text-[#d7ee46] font-medium rounded-[8px] hover:bg-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            Bắt đầu ca →
          </button>
        </div>
      </div>
