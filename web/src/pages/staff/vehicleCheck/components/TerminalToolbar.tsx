      ...
      <div className="flex items-center justify-between px-4 text-[12px] text-[#6b6b6b] h-full min-w-max w-full gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-1.5 whitespace-nowrap text-[11px] transition-colors pb-0.5 ${location.pathname === '/staff'
              ? 'font-bold text-[#1a1a1a] border-b-2 border-[#A3E635]'
              : 'font-medium text-[#6b6b6b] hover:text-[#060606] border-b-2 border-transparent'
              }`}
          >
            <ScanLine className={`w-3.5 h-3.5 ${location.pathname === '/staff' ? 'text-[#65A30D]' : ''
              }`} /> Xe Ra Vào
          </button>
          <button
            onClick={() => navigate('/staff/active-sessions')}
            className={`flex items-center gap-1.5 whitespace-nowrap text-[11px] transition-colors pb-0.5 ${location.pathname.includes('/active-sessions')
              ? 'font-bold text-[#1a1a1a] border-b-2 border-[#A3E635]'
              : 'font-medium text-[#6b6b6b] hover:text-[#060606] border-b-2 border-transparent'
              }`}
          >
            <Car className={`w-3.5 h-3.5 ${location.pathname.includes('/active-sessions') ? 'text-[#65A30D]' : ''
              }`} /> Xe Đang Gửi
          </button>
          <button
            onClick={() => navigate('/staff/exceptions')}
            className={`flex items-center gap-1.5 whitespace-nowrap text-[11px] transition-colors pb-0.5 ${location.pathname.includes('/exceptions')
              ? 'font-bold text-[#1a1a1a] border-b-2 border-[#A3E635]'
              : 'font-medium text-[#6b6b6b] hover:text-[#060606] border-b-2 border-transparent'
              }`}
          >
            <AlertCircle className={`w-3.5 h-3.5 ${location.pathname.includes('/exceptions') ? 'text-[#65A30D]' : ''
              }`} /> Xử Lý Sự Cố
          </button>
          <button
