...
import { Search, AlertTriangle, RefreshCw, CheckCircle, DollarSign, Camera, ScanLine, ImagePlus, X } from "lucide-react";
      ...
      <div>
        ...
        {step === "search" && (
          <form onSubmit={handleSearch} className="space-y-5">
            ...
            >
              {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <><Search className="w-4 h-4" /> Search Session</>}
            </button>
          </form>
        )}
          ...
          return (
            <div className="space-y-4 py-1">
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">Đối chiếu ảnh check-in trước khi xác nhận</p>
              </div>
              ...
              {session.checkInImage ? (
                <div className="rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                  <div className="bg-amber-50 px-3 py-2 border-b border-amber-100 flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-amber-700">Ảnh Chụp Khi Check-in</span>
                    <span className="ml-auto text-[10px] text-amber-500 font-medium">Đối chiếu biển số</span>
                  </div>
                  <img
                    src={session.checkInImage.startsWith('http') ? session.checkInImage : `${SERVER_URL}${session.checkInImage}`}
                    alt="Check-in"
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-2.5">
                ...
                <div className="flex justify-between text-xs border-t pt-2 border-gray-200">
                  ...
                  <span className="font-bold text-gray-800">{fee.details?.durationHours ?? 0} Giờ</span>
                </div>
                {session.slotId && (
                  <div className="flex justify-between text-xs border-t pt-2 border-gray-200">
                    <span className="text-gray-400 font-semibold">VỊ TRÍ</span>
                    <span className="font-bold text-gray-800">
                      {session.floorId?.name && `${session.floorId.name} - `}{session.slotId?.code}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <>Xác Nhận Check-out</>}
                </button>
              </div>
            </div>
          );
