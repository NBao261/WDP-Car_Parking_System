...
export function CheckOutOCR({ step, isMismatch, isNoPlateVehicle, currentSession, ocrPreviewUrl, isUploading, fileInputRef, clearOcrPreview, handleImageUpload }: any) {
  return (
    <div className="flex gap-3 relative shrink-0">
      {step === 'CONFIRM' && !isMismatch && !isNoPlateVehicle && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#A3E635] text-[#1A202C] px-4 py-1 rounded-[4px] font-bold text-[12px] z-10 shadow-sm border border-[#84CC16]">Khớp</div>
      )}
      {step === 'CONFIRM' && isMismatch && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#EF4444] text-white px-4 py-1 rounded-[4px] font-bold text-[12px] z-10 shadow-sm border border-[#DC2626]">Không khớp</div>
      )}
        ...
        <div className="h-[210px] w-full border border-dashed border-[#999] rounded-[6px] flex flex-col items-center justify-center gap-2 bg-[#fdfdfd] overflow-hidden relative">
          {currentSession?.checkInImage ? (
            (() => {
              ...
              if (!imgSrc.startsWith('http')) { imgSrc = `${SERVER_URL}${imgSrc.startsWith('/') ? imgSrc : `/${imgSrc}`}`; }
      ...
      <div className="flex-1 flex flex-col gap-1.5 relative min-h-0">
        ...
        {!ocrPreviewUrl ? (
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="h-[210px] w-full border border-dashed border-[#999] rounded-[6px] flex flex-col items-center justify-center gap-2 hover:border-[#A3E635] hover:bg-[#ECFCCB] transition-all duration-200 disabled:opacity-60 bg-[#fcfcfc]">
            {isUploading ? (<RefreshCw className="w-6 h-6 animate-spin text-[#8bc34a]" />) : (
              <div className="flex flex-col items-center">
                <img src="/Logo_chu.png" alt="LYNC PARK" className="h-16 mb-2 object-contain" />
